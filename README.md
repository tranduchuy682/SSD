...
[***Objective***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#objective)

[***Concepts***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#concepts)

[***Overview***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#overview)

[***Implementation***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#implementation)

[***Training***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#training)

[***Evaluation***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#evaluation)

[***Inference***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#inference)

[***Frequently Asked Questions***](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#faqs)
...
There are more examples at the [end of the tutorial](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#some-more-examples).
...
In this section, I will present an overview of this model. If you're already familiar with it, you can skip straight to the [Implementation](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#implementation) section or the commented code.
...
In the ImageNet VGG-16 [shown previously](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#base-convolutions--part-1), which operates on images of size `224, 224, 3`, you can see that the output of `conv5_3` will be of size `7, 7, 512`. Therefore –
...
[Earlier](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#multibox), we said we would use regression to find the coordinates of an object's bounding box. But then, surely, the priors can't represent our final predicted boxes?
...
This answers the question we posed at the [beginning of this section](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD#a-detour). Considering that each prior is adjusted to obtain a more precise prediction, **these four offsets `(g_c_x, g_c_y, g_w, g_h)` are the form in which we will regress bounding boxes' coordinates**.
...
See `create_data_lists()` in [`utils.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/utils.py).
...
- A **JSON file which contains the `label_map`**, the label-to-index dictionary with which the labels are encoded in the previous JSON file. This dictionary is also available in [`utils.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/utils.py) and directly importable.

#### PyTorch Dataset

See `PascalVOCDataset` in [`datasets.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/datasets.py).

This is a subclass of PyTorch [`Dataset`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/data.html#torch.utils.data.Dataset), used to **define our training and test datasets.** It needs a `__len__` method defined, which returns the size of the dataset, and a `__getitem__` method which returns the `i`th image, bounding boxes of the objects in this image, and labels for the objects in this image, using the JSON files we saved earlier.
...
See `transform()` in [`utils.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/utils.py).
...
The `Dataset` described above, `PascalVOCDataset`, will be used by a PyTorch [`DataLoader`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/data.html#torch.utils.data.DataLoader) in `train.py` to **create and feed batches of data to the model** for training or evaluation.
...
See `VGGBase` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
See `AuxiliaryConvolutions` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
See `PredictionConvolutions` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
See `SSD300` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
See `create_prior_boxes()` under `SSD300` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
See `MultiBoxLoss` in [`model.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/model.py).
...
Before you begin, make sure to save the required data files for training and evaluation. To do this, run the contents of [`create_data_lists.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/create_data_lists.py) after pointing it to the `VOC2007` and `VOC2012` folders in your [downloaded data](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/#download).

See [`train.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/train.py).
...
See [`eval.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/eval.py).
...
We will use `calculate_mAP()` in [`utils.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/utils.py) for this purpose. As is the norm, we will ignore _difficult_ detections in the mAP calculation. But nevertheless, it is important to include them from the evaluation dataset because if the model does detect an object that is considered to be _difficult_, it must not be counted as a false positive.
...
See [`detect.py`](https://github.com/GivralNguyen/learn_computer_vision/tree/main/pytorch/architecture/SSD/detect.py).
